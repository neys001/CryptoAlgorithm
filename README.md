...
```
┌──────────────┬───────────────────────────────────────────┐
│   IV (8 Б)   │  Зашифрованные данные (кратно 8 Б, PKCS5)  │
└──────────────┴───────────────────────────────────────────┘
```
...
fos.write(ivBytes);                          // IV записывается в начало файла
while ((n = fis.read(buffer)) != -1)
    fos.write(cipher.update(buffer, 0, n));  // шифрование по частям
...
> ⚠️ DES считается устаревшим алгоритмом: длина ключа всего 56 бит. Проект сделан в учебных целях, чтобы разобраться, как устроены блочные шифры и режимы их работы. Для реальных задач нужен AES.

## 🛠️ Стек

Java · javax.crypto (JCA) · DES · режим OFB · потоковый ввод-вывод
